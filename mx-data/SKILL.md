...
version: 1.0.0
env:
  - MX_APIKEY: "通过东方财富妙想Skills页面获取的 API Key。注意：2026年5月后获取的Key仅有查询权限，交易接口需要单独申请；若返回status 114说明Key无效或已过期"
---
...
**推荐方式：通过 Python 模块直接调用（最可靠）**

```python
import os, sys
sys.path.insert(0, '/c/Users/June/AppData/Local/hermes/skills/dfcf/mx-data')
# 读取APIKEY（第486行，不要用 enumerate 直接用行号）
lines = open('C:/Users/June/AppData/Local/hermes/.env').readlines()
APIKEY = lines[485].split('=')[1].strip()  # 第486行（0-indexed=485）
os.environ['MX_APIKEY'] = APIKEY

import mx_data
result = mx_data.MXData().query('贵州茅台最新价')
```

> ⚠️ **不要用 `subprocess` / `execute_code` / 内联 `-c "..."` 调用**（execute_code sandbox 环境找不到 Python 路径；MSYS/Git-Bash 内联 `-c` 中 `line.split('=', 1)[1].strip()` 语法会报 `SyntaxError: unmatched ')'`）。正确做法：将脚本写入文件（`/c/Users/June/xxx.py`），然后 `cd /c/Users/June/AppData/Local/hermes/skills/dfcf/mx-data && /c/Python314/python /c/Users/June/xxx.py`。
...
**涨停板数据**：mx_data query 接口**不返回**涨停相关字段（炸板次数、连板数、涨停时间），涨停板数据必须用 akshare `stock_zt_pool_em()`。

**龙虎榜席位明细**：mx_data **不返回**龙虎榜营业部席位数据（push2.eastmoney.com被封禁），量化席位过滤无法通过MX实现。
